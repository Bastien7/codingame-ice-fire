...
fun buildMine(mine: MineSpot) = print("BUILD MINE ${mine.position.x} ${mine.position.y};")
...
fun Game.findMineSpotAvailable(avaiblesCases: List<Case>): MineSpot? {
    return this.mineSpots.firstOrNull { mine -> this.get(mine.position).type == ALLY_ENABLED && avaiblesCases.any { it.position == mine.position } }
}

fun Game.findCaseToTrain(avaiblesCases: List<Case>, enemyQG: Building): MutableList<Case> {
    return this.cases.asSequence().flatten()
        .filter { case -> case.type == ALLY_ENABLED }
        .map { it.adjacentCase(this) }
        .flatten()
        .filter { it in avaiblesCases}
        .sortedBy { it.position.distanceTo(enemyQG.position) }
        .toMutableList()
}

fun Game.findCaseToBuildTower(avaiblesCases: List<Case>, myTowers: List<Building>, myPlayerQG: Building): Case? {
    ...
    return (this.cases.flatten() - towerCases)
        .filter { case -> case.type == ALLY_ENABLED && case in avaiblesCases }
...
fun main(args: Array<String>) {
    ...
    while (true) {
        game.update(input)
        val myPlayerQG = game.buildings.first { it.owner == myPlayer && it.type == BuildingType.QG }
        ...
        val avaiblesCases = game.cases.flatten()
            .filter { it.type != VOID && myPlayer.units.noneIn(it) && enemy.units.noneIn(it) && game.buildings.noneOn(it) && myPlayerQG.position != it.position }

        val casesToTrain = game.findCaseToTrain(avaiblesCases, enemyQG)
        val casesToBuildTower = game.findCaseToBuildTower(avaiblesCases, myPlayerTowers, myPlayerQG)
        val mineToBuild = game.findMineSpotAvailable(avaiblesCases)

package savePotential


import savePotential.BuildingType.*
import savePotential.CaseType.*
import java.util.*
...
class StrategyConquer(game: Game, private val explorers: MutableList<Unit> = mutableListOf()) : Strategy(game) {
    override fun play(turn: Int) {
        ...
        var casesToTrain = game.findCaseToTrain(enemyQG)
        ...
        while (explorers.size < 7 && casesToTrain.isNotEmpty() && myPlayer.gold >= 10) {
            ...
            myPlayer.gold -= 10
            myPlayer.income -= 1
            casesToTrain = game.findCaseToTrain(enemyQG)
        }
